# 011 - Cloud Deployment Plan

## 目標

將 LedgerOne 部署到雲端，實現遠端存取，並透過 MCP 讓 AI 應用（Claude Desktop/App）可直接連線記帳。
...
3. **環境變數**：敏感資訊存在各平台的 Environment Variables（不進 Git）
4. **CORS**：僅允許 Frontend domain
5. **Rate Limiting**：建議新增 API rate limiting（防止濫用）
6. **Cloudflare WAF**：免費包含基本 Web Application Firewall
