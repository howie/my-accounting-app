...
Password format: ID number (e.g., A123456789)
Email sender: service@email.cathaybk.com.tw
...
@register_parser
class CathayParser(BankStatementParser):
    """Parser for Cathay United Bank credit card statements."""

    bank_code = "CATHAY"
    bank_name = "國泰世華"
    email_query = "from:service@email.cathaybk.com.tw subject:信用卡電子帳單"
    password_hint = "身分證字號（例：A123456789）"

    def parse_statement(self, pdf_content: PdfContent) -> list[ParsedStatementTransaction]:
        """Parse Cathay credit card statement.

        Cathay statements typically have:
        - Transaction date, posting date
        - Merchant name with location
        - Amount in TWD or foreign currency
        """
        transactions = []
    ...
    def _parse_table(self, table: list[list[str]]) -> list[ParsedStatementTransaction]:
        ...
        for i, row in enumerate(table):
            ...
            if "交易日" in row_text or "消費日" in row_text or "日期" in row_text:
                header_row = i
                for j, cell in enumerate(row):
                    cell_text = str(cell).strip()
                    if "交易日" in cell_text or "消費日" in cell_text or cell_text == "日期":
                        date_col = j
                    elif "說明" in cell_text or "交易說明" in cell_text or "商店" in cell_text:
                        desc_col = j
                    elif "金額" in cell_text or "新臺幣" in cell_text:
                        amount_col = j
                break
    ...
    def _parse_text(self, text: str) -> list[ParsedStatementTransaction]:
        ...
        # Cathay statement patterns
        patterns = [
            # MM/DD format
            r"(\d{1,2}/\d{1,2})\s+(.+?)\s+([\d,]+(?:\.\d{2})?)\s*$",
            # YYYY/MM/DD format
            r"(\d{4}/\d{1,2}/\d{1,2})\s+(.+?)\s+([\d,]+(?:\.\d{2})?)\s*$",
            # Compact format with date at start
            r"^(\d{2}/\d{2})\s*(.{10,50}?)\s+([\d,]+)$",
        ]
    ...
    def _parse_amount(self, amount_str: str) -> Decimal | None:
        ...
        cleaned = re.sub(r"[NT$,\s元]", "", amount_str)
        ...
        for pattern in patterns:
            ...
            if match:
                if len(match.groups()) == 2:
                    ...
                    # Single date - assume it's the end of billing period
                    end_date = self._parse_date(match.group(1))
                    if end_date:
                        # Estimate start as ~30 days before
