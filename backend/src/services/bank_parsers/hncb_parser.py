"""Hua Nan Commercial Bank (華南銀行) credit card statement parser.

Password format: Birth date YYYYMMDD (e.g., 19900520)
Email sender: creditcard@hncb.com.tw
...
@register_parser
class HncbParser(BankStatementParser):
    """Parser for Hua Nan Commercial Bank credit card statements."""

    bank_code = "HNCB"
    bank_name = "華南銀行"
    email_query = "from:creditcard@hncb.com.tw subject:信用卡電子帳單"
    password_hint = "出生年月日 YYYYMMDD（例：19900520）"

    def parse_statement(self, pdf_content: PdfContent) -> list[ParsedStatementTransaction]:
        """Parse HNCB credit card statement."""
        transactions = []
    ...
    def _parse_text(self, text: str) -> list[ParsedStatementTransaction]:
        ...
        patterns = [
            ...
            # ROC year format (民國年)
            r"(\d{2,3}/\d{1,2}/\d{1,2})\s+(.+?)\s+([\d,]+(?:\.\d{2})?)\s*$",
        ]
