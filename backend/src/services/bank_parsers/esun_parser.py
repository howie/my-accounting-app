"""E.SUN Bank (玉山銀行) credit card statement parser.

Password format: Birth date MMDD + ID last 4 digits (e.g., 05206789)
Email sender: service@email.esunbank.com.tw
...
@register_parser
class EsunParser(BankStatementParser):
    """Parser for E.SUN Bank credit card statements."""

    bank_code = "ESUN"
    bank_name = "玉山銀行"
    email_query = "from:service@email.esunbank.com.tw subject:信用卡電子帳單"
    password_hint = "生日MMDD + 身分證末4碼（例：05206789）"

    def parse_statement(self, pdf_content: PdfContent) -> list[ParsedStatementTransaction]:
        """Parse E.SUN credit card statement."""
        transactions = []
            ...
            if "交易日" in row_text or "消費日" in row_text:
                ...
                for j, cell in enumerate(row):
                    ...
                    if "交易日" in cell_text or "消費日" in cell_text:
                        date_col = j
                    elif "說明" in cell_text or "摘要" in cell_text or "商店" in cell_text:
                        desc_col = j
