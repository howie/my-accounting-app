"""HSBC Bank (匯豐銀行) credit card statement parser.

Password format: Birth date MMDD (e.g., 0520)
Email sender: noreply@hsbc.com.tw
...
class HsbcParser(BankStatementParser):
    """Parser for HSBC Bank credit card statements."""

    bank_code = "HSBC"
    bank_name = "匯豐銀行"
    email_query = "from:noreply@hsbc.com.tw subject:信用卡電子帳單"
    password_hint = "生日MMDD（例：0520）"

    def parse_statement(self, pdf_content: PdfContent) -> list[ParsedStatementTransaction]:
        """Parse HSBC credit card statement."""
        transactions = []
    ...
    def _parse_table(self, table: list[list[str]]) -> list[ParsedStatementTransaction]:
        ...
        for i, row in enumerate(table):
            ...
            if "date" in row_text or "交易日" in row_text:
                header_row = i
                for j, cell in enumerate(row):
                    cell_text = str(cell).strip().lower()
                    if cell_text in ("date", "交易日", "消費日"):
                        date_col = j
                    elif "description" in cell_text or "說明" in cell_text:
                        desc_col = j
                    elif "amount" in cell_text or "金額" in cell_text:
                        amount_col = j
                break
    ...
    def _parse_text(self, text: str) -> list[ParsedStatementTransaction]:
        ...
        # HSBC may use DD/MM or DD MMM format
        patterns = [
    ...
    def _parse_date(self, date_str: str) -> date | None:
        ...
        if match:
            ...
            if month_str in months:
                try:
                    return date(date.today().year, months[month_str], day)
                except ValueError:
                    return None
    ...
    def extract_billing_period(self, pdf_content: PdfContent) -> tuple[date | None, date | None]:
        ...
        patterns = [
            r"Statement Period[：:]\s*(\d{1,2}\s+[A-Za-z]{3})\s*[~to-]\s*(\d{1,2}\s+[A-Za-z]{3})",
