"""SinoPac Bank (永豐銀行) credit card statement parser.

Password format: ID last 4 digits + birth MMDD (e.g., 67890520)
Email sender: service@sinopac.com
...
@register_parser
class SinopacParser(BankStatementParser):
    """Parser for SinoPac Bank credit card statements."""

    bank_code = "SINOPAC"
    bank_name = "永豐銀行"
    email_query = "from:service@sinopac.com subject:信用卡電子帳單"
    password_hint = "身分證末4碼 + 生日MMDD（例：67890520）"

    def parse_statement(self, pdf_content: PdfContent) -> list[ParsedStatementTransaction]:
        """Parse SinoPac credit card statement."""
        transactions = []
