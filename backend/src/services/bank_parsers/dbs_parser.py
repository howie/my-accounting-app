"""DBS Bank (星展銀行) credit card statement parser.

Password format: Birth date YYYYMMDD (e.g., 19900520)
Email sender: estatement@dbs.com
...
@register_parser
class DbsParser(BankStatementParser):
    """Parser for DBS Bank credit card statements."""

    bank_code = "DBS"
    bank_name = "星展銀行"
    email_query = "from:estatement@dbs.com subject:信用卡電子帳單"
    password_hint = "出生年月日 YYYYMMDD（例：19900520）"

    def parse_statement(self, pdf_content: PdfContent) -> list[ParsedStatementTransaction]:
        """Parse DBS credit card statement."""
        transactions = []
        ...
        for i, row in enumerate(table):
            ...
            if "date" in row_text or "交易日" in row_text:
                ...
                for j, cell in enumerate(row):
                    ...
                    if "date" in cell_text or "交易日" in cell_text:
                        date_col = j
    ...
    def _parse_text(self, text: str) -> list[ParsedStatementTransaction]:
        ...
        # DBS often uses DD MMM format (e.g., "15 Jan")
        patterns = [
